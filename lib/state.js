...
function filterStateByBuildings(state, buildingIds = []) {
  const normalized = cleanupStateRelationships(normalizeState(state));
  const allowedBuildingIds = new Set((Array.isArray(buildingIds) ? buildingIds : []).map(sanitizeText).filter(Boolean));

  if (!allowedBuildingIds.size) {
    return {
      ...buildEmptyState(normalized.settings.companyName || "RentHome"),
      settings: { ...normalized.settings }
    };
  }

  const buildings = normalized.buildings.filter((building) => allowedBuildingIds.has(building.id));
  const unitIds = new Set(normalized.units.filter((unit) => allowedBuildingIds.has(unit.buildingId)).map((unit) => unit.id));
  const units = normalized.units.filter((unit) => unitIds.has(unit.id));
  const leaseIds = new Set(normalized.leases.filter((lease) => unitIds.has(lease.unitId)).map((lease) => lease.id));
  const leases = normalized.leases.filter((lease) => leaseIds.has(lease.id));
  const tenantIds = new Set([
    ...leases.map((lease) => lease.tenantId).filter(Boolean),
    ...normalized.tenants.filter((tenant) => tenant.unitId && unitIds.has(tenant.unitId)).map((tenant) => tenant.id)
  ]);
  const tenants = normalized.tenants.filter((tenant) => tenantIds.has(tenant.id));
  const meterIds = new Set(normalized.meters.filter((meter) => unitIds.has(meter.unitId)).map((meter) => meter.id));
  const chargeIds = new Set(normalized.charges.filter((charge) => leaseIds.has(charge.leaseId)).map((charge) => charge.id));
  ...
  return {
    ...
    tenants,
    leases,
    meters: normalized.meters.filter((meter) => meterIds.has(meter.id)),
    meterReadings: normalized.meterReadings.filter((reading) => meterIds.has(reading.meterId)),
    charges: normalized.charges.filter((charge) => chargeIds.has(charge.id)),
    payments: normalized.payments.filter((payment) => leaseIds.has(payment.leaseId) || chargeIds.has(payment.chargeId)),
    expenses: normalized.expenses.filter((expense) => (!expense.buildingId || allowedBuildingIds.has(expense.buildingId)) && (!expense.unitId || unitIds.has(expense.unitId))),
    tickets: normalized.tickets.filter((ticket) => unitIds.has(ticket.unitId) || tenantIds.has(ticket.tenantId)),
    documents: normalized.documents.filter((document) => (!document.tenantId || tenantIds.has(document.tenantId)) && (!document.leaseId || leaseIds.has(document.leaseId)) && (!document.unitId || unitIds.has(document.unitId))),
    tasks: normalized.tasks.filter((task) => (!task.tenantId || tenantIds.has(task.tenantId)) && (!task.leaseId || leaseIds.has(task.leaseId)) && (!task.unitId || unitIds.has(task.unitId))),
    generatedDocuments: normalized.generatedDocuments.filter((document) => (!document.tenantId || tenantIds.has(document.tenantId)) && (!document.leaseId || leaseIds.has(document.leaseId)) && (!document.unitId || unitIds.has(document.unitId))),
    messages: normalized.messages.filter((message) => (!message.tenantId || tenantIds.has(message.tenantId)) && (!message.leaseId || leaseIds.has(message.leaseId)) && (!message.unitId || unitIds.has(message.unitId))),
    taxAdjustments: normalized.taxAdjustments.filter((entry) => !entry.unitId || unitIds.has(entry.unitId)),
    taxPayments: normalized.taxPayments.filter((entry) => !entry.unitId || unitIds.has(entry.unitId)),
    taxAnnualClosings: normalized.taxAnnualClosings.filter((entry) => !entry.unitId || unitIds.has(entry.unitId)),
    settlements: normalized.settlements.filter((settlement) => (!settlement.tenantId || tenantIds.has(settlement.tenantId)) && (!settlement.leaseId || leaseIds.has(settlement.leaseId)) && (!settlement.unitId || unitIds.has(settlement.unitId))),
    periodSettlements: normalized.periodSettlements.filter((settlement) => (!settlement.tenantId || tenantIds.has(settlement.tenantId)) && (!settlement.leaseId || leaseIds.has(settlement.leaseId)) && (!settlement.unitId || unitIds.has(settlement.unitId))),
    settings: { ...normalized.settings }
  };
}

function mergeScopedState(baseState, scopedState, buildingIds = []) {
  const normalizedBase = cleanupStateRelationships(normalizeState(baseState));
  const baseScope = filterStateByBuildings(normalizedBase, buildingIds);
  const incomingScope = filterStateByBuildings(scopedState, buildingIds);
  const nextState = { ...normalizedBase };
  const replaceCollection = (key) => {
    const scopedIds = uniqueIds(baseScope[key]);
    nextState[key] = normalizedBase[key].filter((item) => !scopedIds.has(item?.id)).concat(incomingScope[key] || []);
  };
  ...
  [
    ...
    "settlements",
    "periodSettlements"
  ].forEach(replaceCollection);

  return cleanupStateRelationships(normalizeState(nextState, normalizedBase.settings.companyName || "RentHome"));
}
